...
# Creates a table named 'books'
	...
	"""
    isbn = db.Column(
        db.Integer, primary_key=True)  # only use 13 digit isbn, 10 digit if no 13
...
"""
    ...
    def post(self, isbns):
        ...
        imagelinks = ast.literal_eval(
            data["imageLinks"][0])  # dumb json parsing
        thumbnail = imagelinks["thumbnail"]  # dumb json parsing.
        smallThumbnail = imagelinks["smallThumbnail"]  # dumb json parsing
        book = BookModel(data['title'], data['subtitle'], authors, isbn, categories, data['publishedDate'],
                         smallThumbnail, thumbnail, data['previewLink'], data['infoLink'], data['canonicalVolumeLink'])
        book.save_to_db()
...
"""
    ...
    def get(self, search):  # books -> users
        ...
        print(search)
        books = BookModel.query.filter(or_(BookModel.title.contains(search), BookModel.authors.contains(search), BookModel.subtitle.contains(search), BookModel.categories.contains(
            search), BookModel.publishedDate.contains(search)))  # Operator 'contains' is not supported on this expression!!! search in subtitle, search in publishedDate
